#!/usr/bin/env bash
# Copyright (c) 2016 The PaydayCoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   paydaycoincore.org and paydaycoin.org and compares them.
...
WORKINGDIR="/tmp/paydaycoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://paydaycoincore.org"
HOST2="https://paydaycoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="paydaycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'paydaycoin-',
...
if [ $? -ne 0 ]; then
   echo "paydaycoin.org failed to provide signature file, but paydaycoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "paydaycoin.org and paydaycoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the PaydayCoin Core binary release signing key installed?"
   fi
