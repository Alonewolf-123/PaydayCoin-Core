...
- [Development guidelines](#development-guidelines)
    - [General PaydayCoin Core](#general-paydaycoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
PaydayCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play paydaycoins" on the test network, if you
...
PaydayCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/paydaycoin/paydaycoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_paydaycoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_paydaycoin --log_level=test_suite
$ valgrind -v --leak-check=full src/paydaycoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_paydaycoin.coverage/index.html`.
```
...
To profile a running paydaycoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep paydaycoind` -- sleep 60
```
...
PaydayCoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/paydaycoin/paydaycoin/issues/12691)
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8123.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8124 for connections and services them.
...
However, in open source software such as PaydayCoin Core, where everyone uses
...
pay attention to for reviewers of PaydayCoin Core code.

General PaydayCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: PaydayCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `PAYDAYCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef PAYDAYCOIN_FOO_BAR_H
#define PAYDAYCOIN_FOO_BAR_H
...
#endif // PAYDAYCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of PaydayCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against PaydayCoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/paydaycoin-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/paydaycoin-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/paydaycoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors it will cause problems with PaydayCoin's `select()` loop, because
...
compatibility between nodes. This happened in PaydayCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to PaydayCoin's
...
Commit [`bb81e173`](https://github.com/paydaycoin/paydaycoin/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `paydaycoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `paydaycoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of PaydayCoin Core, the wallet was always
