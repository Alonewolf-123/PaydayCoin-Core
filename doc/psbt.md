# PSBT Howto for PaydayCoin Core

Since PaydayCoin Core 0.17, an RPC interface exists for Partially Signed PaydayCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/paydaycoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for PaydayCoin transactions that are not fully signed
...
#### Multisig with multiple PaydayCoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
PaydayCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `paydaycoin-cli` is used.
...
Later, when *V* PDAY has been received on *Amulti*, and Bob and Carol want to
