...
`test/functional/test_framework/test_framework.py:PaydayCoinTestFramework.main`.
...
def main():
    ...
    enable_paydaycoind = config["components"].getboolean("ENABLE_PAYDAYCOIND")

    if not enable_paydaycoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci):
    ...
    # Warn if paydaycoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "paydaycoind"]) is not None:
            print("%sWARNING!%s There is already a paydaycoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `paydaycoin-cli help` (`rpc_interface.txt`).
