...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
    ...
    if(fZMQWalletRawTx)
    {
        ...
        if(retString.size() > 10)
            GetMainSignals().NotifyPaydayCoinUpdate(ret.write().c_str(), "walletrawtx");
    }
    return true;
}
...
bool CWallet::IsAssetMine(const CTransaction& tx, const isminefilter& filter) const {
    if(tx.nVersion == PAYDAYCOIN_TX_VERSION_ASSET_SEND || tx.nVersion == PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_SEND){
    ...
    while (block_height && !fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (chain().findBlock(block_hash, &block) && !block.IsNull()) {
            ...
            if (!locked_chain->getBlockHeight(block_hash)) {
                ...
                // https://github.com/paydaycoin/paydaycoin/pull/14711#issuecomment-458342518
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-paydaycoin-address
 ...
 * https://paydaycointalk.org/?topic=54527, or
 * https://github.com/paydaycoin/paydaycoin/pull/1393.
 */
...
void CWallet::LockMasternodeOutputs(){
    ...
    if(fZMQWalletStatus){
        ...
        GetMainSignals().NotifyPaydayCoinUpdate(oWalletState.write().c_str(), "walletstatus");
    }
}
