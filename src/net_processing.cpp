...
/** PAYDAYCOIN How many microseconds to delay requesting transactions from inbound peers */
...
{
    // PAYDAYCOIN
    return ::ChainActive().Tip()->GetBlockTime() > GetAdjustedTime() - consensusParams.nPowTargetSpacing * 200;
}
...
void RequestTx(CNodeState* state, const CInv& inv, int64_t nNow) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    int64_t process_time = CalculateTxGetDataTime(inv.hash, nNow, !state->fPreferredDownload);
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // PAYDAYCOIN
    /* 
        PaydayCoin Related Inventory Messages
...
void static ProcessGetData(CNode* pfrom, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc) LOCKS_EXCLUDED(cs_main)
{
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        it->type == MSG_MASTERNODE_PAYMENT_BLOCK || it->type == MSG_MASTERNODE_ANNOUNCE || it->type == MSG_MASTERNODE_PING || it->type == MSG_GOVERNANCE_OBJECT ||
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
...

    {
        ...
        if (pfrom->nVersion >= SIZE_HEADERS_LIMIT_VERSION
            ...
            maxSize = true;
        // FIXME: This change (with hasNewHeaders) is rolled back in PaydayCoin,
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &fMissingInputs2, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetReason() != ValidationInvalidReason::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/paydaycoin/paydaycoin/issues/8279 for details.
                assert(recentRejects);
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
		...
		std::vector<CInv> vInv;
		{
			...
			bool fSendTrickle = true;/*pto->fWhitelisted;
			...
			if (fSendTrickle) {
				...
				LOCK(pto->cs_filter);
				while (!vInvTx.empty() && nRelayedTransactions < 3000) {
		...
		// PAYDAYCOIN Send non-tx/non-block inventory items
        ...
        while (!tx_process_time.empty() && tx_process_time.begin()->first <= nNow && state.m_tx_download.m_tx_in_flight.size() < MAX_PEER_TX_IN_FLIGHT) {
            ...
            if (!AlreadyHave(inv)) {
                ...
                if (last_request_time <= nNow - GETDATA_TX_INTERVAL) {
                    ...
                    // PAYDAYCOIN
                    int64_t next_process_time = CalculateTxGetDataTime(inv.hash, nNow, !state.fPreferredDownload);
