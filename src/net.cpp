...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (PaydayCoin users should be well used to small numbers with many decimal places by now :)
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    // PAYDAYCOIN don't accept incoming connections until fully synced
    ...
    if(fMasternodeMode && !fGethSynced && !fUnitTest && !fTPSTest) {
    ...
    if(fZMQNetworkStatus){
        ...
        GetMainSignals().NotifyPaydayCoinUpdate(oNetworkStatus.write().c_str(), "networkstatus");
    } 
}
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "PaydayCoin " + FormatFullVersion();
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    // PAYDAYCOIN Initiate masternode connections
