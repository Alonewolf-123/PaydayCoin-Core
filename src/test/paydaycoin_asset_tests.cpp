...
BOOST_FIXTURE_TEST_SUITE (paydaycoin_asset_tests, BasicPaydayCoinTestingSetup)
...
BOOST_AUTO_TEST_CASE(generate_asset_audittxroot)
{
    ...
    UniValue r;
    BOOST_CHECK_NO_THROW(CallRPC("node1", "paydaycoinsetethheaders \"[[709780,\\\"709780\\\",\\\"709779\\\",\\\"a\\\",\\\"a\\\"],[707780,\\\"707780\\\",\\\"707779\\\",\\\"a\\\",\\\"a\\\"],[707772,\\\"707772\\\",\\\"707771\\\",\\\"a\\\",\\\"a\\\"],[707776,\\\"707776\\\",\\\"707775\\\",\\\"a\\\",\\\"a\\\"],[707770,\\\"707770\\\",\\\"707769\\\",\\\"a\\\",\\\"a\\\"],[707778,\\\"707778\\\",\\\"707777\\\",\\\"a\\\",\\\"a\\\"],[707774,\\\"707774\\\",\\\"707773\\\",\\\"a\\\",\\\"a\\\"]]\""));
    int64_t start = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoinsetethstatus synced 709780"));
    int64_t end = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    printf("paydaycoinsetethstatus elasped time %lld\n", end-start);
    ...
    BOOST_CHECK_NO_THROW(CallRPC("node1", "paydaycoinsetethheaders \"[[707773,\\\"707773\\\",\\\"707772\\\",\\\"a\\\",\\\"a\\\"],[707775,\\\"707775\\\",\\\"707774\\\",\\\"a\\\",\\\"a\\\"],[707771,\\\"707771\\\",\\\"707770\\\",\\\"a\\\",\\\"a\\\"],[707777,\\\"707777\\\",\\\"707776\\\",\\\"a\\\",\\\"a\\\"],[707779,\\\"707779\\\",\\\"707778\\\",\\\"a\\\",\\\"a\\\"],[707781,\\\"707781\\\",\\\"707780\\\",\\\"a\\\",\\\"a\\\"]]\""));
    ...
    printf("paydaycoinsetethstatus1 elasped time %lld\n", end-start);
    ...
    BOOST_CHECK_NO_THROW(CallRPC("node1", "paydaycoinsetethheaders \"[[707773,\\\"707773\\\",\\\"707771\\\",\\\"a\\\",\\\"a\\\"],[707775,\\\"707775\\\",\\\"707773\\\",\\\"a\\\",\\\"a\\\"]]\""));
    start = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoinsetethstatus synced 709780"));
    end = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    printf("paydaycoinsetethstatus2 elasped time %lld\n", end-start);
    blocksArray = find_value(r.get_obj(), "missing_blocks").get_array();
...
BOOST_AUTO_TEST_CASE(generate_asset_audittxroot1)
{
    ...
    for(unsigned int i = nStartHeight;i<(nStartHeight+120000);i++){
        ...
        if(nCount > 0 && (nCount % 4000) == 0){
            BOOST_CHECK_NO_THROW(CallRPC("node1", "paydaycoinsetethheaders \"[" + roots + "]\""));
            roots = "";
            nCount = 0;
            continue;
        }
    ...
    if(!roots.empty())
        BOOST_CHECK_NO_THROW(CallRPC("node1", "paydaycoinsetethheaders \"[" + roots + "]\""));
    int64_t start = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoinsetethstatus synced 820000"));
    int64_t end = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    printf("paydaycoinsetethstatus elasped time %lld\n", end-start);
    UniValue blocksArray = find_value(r.get_obj(), "missing_blocks").get_array();
    ...
    BOOST_CHECK_NO_THROW(CallRPC("node1", "paydaycoinsetethheaders \"[[814011,\\\"814011\\\",\\\"814010\\\",\\\"a\\\",\\\"a\\\"],[700059,\\\"700059\\\",\\\"700058\\\",\\\"a\\\",\\\"a\\\"],[800022,\\\"800022\\\",\\\"800021\\\",\\\"a\\\",\\\"a\\\"]]\""));
    start = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoinsetethstatus synced 820000"));
    end = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    printf("paydaycoinsetethstatus1 elasped time %lld\n", end-start);
...
BOOST_AUTO_TEST_CASE(generate_paydaycoinmint)
{
    UniValue r;
    printf("Running generate_paydaycoinmint...\n");  
    ...
    PaydayCoinMint("node1", newaddress, amount, height, spv_tx_value, spv_tx_root, spv_tx_parent_nodes, spv_tx_path, spv_receipt_value, spv_receipt_root, spv_receipt_parent_nodes);
    
    // try to mint again
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoinmint " + newaddress + " " + amount + " " + itostr(height) + " " + spv_tx_value + " a0" + spv_tx_root + " " + spv_tx_parent_nodes + " " + spv_tx_path + " " + spv_receipt_value + " a0" + spv_receipt_root + " " + spv_receipt_parent_nodes +  " ''"));
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransactionwithwallet " + find_value(r.get_obj(), "hex").get_str()));
    string hex_str = find_value(r.get_obj(), "hex").get_str();
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin)
{
    printf("Running generate_burn_paydaycoin...\n");
    UniValue r;
    string newaddress = GetNewFundedAddress("node1");
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoinburn " + newaddress + " 9.9 0x931D387731bBbC988B312206c74F77D004D6B84b"));
    
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransactionwithwallet " + find_value(r.get_obj(), "hex").get_str()));
    string hexStr = find_value(r.get_obj(), "hex").get_str();    
    BOOST_CHECK_NO_THROW(r = CallRPC("node1", "sendrawtransaction " + hexStr, true, false));
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "paydaycoindecoderawtransaction " + hexStr));
	BOOST_CHECK_EQUAL(find_value(r.get_obj(), "txtype").get_str(), "paydaycoinburn");
    GenerateBlocks(5, "node1");
    ...
    BOOST_CHECK_THROW(r = CallRPC("node1", "paydaycoinburn " + newaddress + " 0.1 0x931D387731bBbC988B312206c74F77D004D6B84b"), runtime_error);
}
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset_multiple)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset_multiple...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset_zdag)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset_zdag...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset_zdag1)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset_zdag1...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset_zdag2)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset_zdag2...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset_zdag3)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset_zdag3...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
...
BOOST_AUTO_TEST_CASE(generate_burn_paydaycoin_asset_zdag4)
{
    UniValue r;
    printf("Running generate_burn_paydaycoin_asset_zdag4...\n");
    GenerateBlocks(5);
    GenerateBlocks(5, "node2");
    GenerateBlocks(5, "node3");
