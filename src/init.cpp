 ...
 */
static const char* PAYDAYCOIN_PID_FILENAME = "paydaycoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PAYDAYCOIN_PID_FILENAME)));
}
    ...
    if(fMasternodeMode)
    {
        ...
        while(ifs >> pidFile){
            if(pidFile && pidFile != getpid()){
                try{
                    KillProcess(pidFile);
                    LogPrintf("%s: PaydayCoind successfully exited from pid %d(from paydaycoind.pid)\n", __func__, pidFile);
                }
                catch(...){
                    LogPrintf("%s: PaydayCoind failed to exit from pid %d(from paydaycoind.pid)\n", __func__, pidFile);
                }
            } 
        } 
        ifs.close();
 ...
 * between paydaycoind, and paydaycoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", PAYDAYCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", PAYDAYCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    // PAYDAYCOIN
    gArgs.AddArg("-gethwebsocketport=<port>", strprintf("Listen for GETH Web Socket connections on <port> for the relayer (default: %u)", 8717), false, OptionsCategory::RPC);
    ...
    gArgs.AddArg("-litemode=<n>", strprintf("Disable all PaydayCoin specific functionality (Masternodes, Governance) (0-1, default: 0)"), false, OptionsCategory::OPTIONS);
    gArgs.AddArg("-sporkaddr=<hex>", strprintf("Override spork address. Only useful for regtest. Using this on mainnet or testnet will ban you."), false, OptionsCategory::OPTIONS); 
    gArgs.AddArg("-masternode=<n>", strprintf("Enable the client to act as a masternode (0-1, default: 0)"), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-assetindex=<n>", strprintf("Index PaydayCoin Assets for historical information (0-1, default: 0)"), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/paydaycoin/paydaycoin>";
    const std::string URL_WEBSITE = "<https://paydaycoin.org>";
    ...
    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 2009, COPYRIGHT_YEAR) + " ") + "\n" +
           ...
           "\n" +
           strprintf(_("PaydayCoin is open source software produced by a global network of developers. By downloading, distributing and using PaydayCoin and the PaydayCoin network you release the developers involved in the PaydayCoin Project past, present, and future from any and all liability. You are responsible for your creations on the PaydayCoin network. You agree that the developers of the PaydayCoin Project carry no responsibility for the actions/data or entities of *any* definition created on the network by yourself or others on the network to which you may be exposed.")) + "\n" +
           strprintf(_("The developers of the PaydayCoin Project do not have the power to modify data on the PaydayCoin network, it is backed by an immutable blockchain, which you further acknowledge through use of PaydayCoin, the PaydayCoin network, and PaydayCoin services. If you do not agree to these terms, please refrain from using PaydayCoin and its related services.")) +
           "\n";
}
...
/** Sanity checks
 *  Ensure that PaydayCoin is running in a usable environment with all
...
{
    // Make sure only a single PaydayCoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", PAYDAYCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if paydaycoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if paydaycoin is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // we don't need to ever reset the txroots db because it is an external chain not related to paydaycoin chain
    ...
    if(fMasternodeMode) {
        ...
        std::string result;
        std::unique_ptr<FILE, decltype(&pclose)> pipe(popen("pidof paydaycoind | wc -w", "r"), pclose);
        if (!pipe) {
        ...
        if(resultInt != 1)   
            return InitError(_("Ensure you are running this masternode in a Unix OS and that only on paydaycoind is running...")); 
    ...
    // PAYDAYCOIN ********************************************************* Step 11b: Load cache data
   ...
   // force UpdatedBlockTip to initialize nCachedBlockHeight for DS, MN payments and budgets
    ...
    // ********************************************************* Step 11d: schedule PaydayCoin-specific tasks
