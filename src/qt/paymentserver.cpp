...
const int PAYDAYCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString PAYDAYCOIN_IPC_PREFIX("paydaycoin:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/paydaycoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/paydaycoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/paydaycoin-paymentrequest";
#endif
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the paydaycoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(PAYDAYCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // paydaycoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parsePaydayCoinURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(PAYDAYCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(PAYDAYCOIN_IPC_CONNECT_TIMEOUT);
...
#endif
{
    ...
    // on Mac: sent when you click paydaycoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start paydaycoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling paydaycoin: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("paydaycoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'paydaycoin://' is not a valid URI. Use 'paydaycoin:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(PAYDAYCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // paydaycoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parsePaydayCoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid PaydayCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in paydaycoin: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom paydaycoin addresses are not supported
        ...
        // PaydayCoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(PaydayCoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
