...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "outputs") {
        ...
        std::vector<COutput> vPossibleCoinsLevel1;
        std::vector<COutput> vPossibleCoinsLevel2;
        std::vector<COutput> vPossibleCoinsLevel3;
        std::vector<COutput> vPossibleCoinsLevel4;
        auto locked_chain = pwallet->chain().lock();
        pwallet->AvailableCoins(*locked_chain, vPossibleCoinsLevel1, true, nullptr, Params().MasternodeCollateralLevel1(), Params().MasternodeCollateralLevel1(), MAX_MONEY, 0, 0, 9999999, true);
        pwallet->AvailableCoins(*locked_chain, vPossibleCoinsLevel2, true, nullptr, Params().MasternodeCollateralLevel2(), Params().MasternodeCollateralLevel2(), MAX_MONEY, 0, 0, 9999999, true);
        pwallet->AvailableCoins(*locked_chain, vPossibleCoinsLevel3, true, nullptr, Params().MasternodeCollateralLevel3(), Params().MasternodeCollateralLevel3(), MAX_MONEY, 0, 0, 9999999, true);
        pwallet->AvailableCoins(*locked_chain, vPossibleCoinsLevel4, true, nullptr, Params().MasternodeCollateralLevel4(), Params().MasternodeCollateralLevel4(), MAX_MONEY, 0, 0, 9999999, true);
   
        UniValue obj(UniValue::VOBJ);
        for (const auto& out : vPossibleCoinsLevel1) {
            obj.pushKV(out.tx->GetHash().ToString(), strprintf("%d", out.i));
        }
        for (const auto& out : vPossibleCoinsLevel2) {
            obj.pushKV(out.tx->GetHash().ToString(), strprintf("%d", out.i));
        }
        for (const auto& out : vPossibleCoinsLevel3) {
            obj.pushKV(out.tx->GetHash().ToString(), strprintf("%d", out.i));
        }
        for (const auto& out : vPossibleCoinsLevel4) {
            obj.pushKV(out.tx->GetHash().ToString(), strprintf("%d", out.i));
        }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.pushKV("payee", EncodeDestination(PKHash(mn.pubKeyCollateralAddress)));
        }
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
            RPCHelpMan{"masternodelist",    
                ...
                "  payee          - Print PaydayCoin address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               EncodeDestination(PKHash(mn.pubKeyCollateralAddress)) << " " <<
                ...
                objMN.pushKV("payee", EncodeDestination(PKHash(mn.pubKeyCollateralAddress)));
                objMN.pushKV("status", mn.GetStatus());
                objMN.pushKV("level", CMasternode::GetLevel(mn.nLevel));
                ...
                std::string strPayee = EncodeDestination(PKHash(mn.pubKeyCollateralAddress));
    ...
    if (strCommand == "decode")
    {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.pushKV("pubKeyCollateralAddress", EncodeDestination(PKHash(mnb.pubKeyCollateralAddress)));
