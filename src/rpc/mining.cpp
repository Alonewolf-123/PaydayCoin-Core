...
{
    static const int nInnerLoopCount = 0x0fffffff;
    int nHeightEnd = 0;
    int nHeight = 0;
        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated paydaycoin to."},
            ...
            + "If you are running the paydaycoin core wallet, you can get a new address to send the newly generated paydaycoin to with:\n"
...
// NOTE: Unlike wallet RPC (which use PDAY values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"getblocktemplate",
                ...
                "    https://github.com/paydaycoin/bips/blob/master/bip-0022.mediawiki\n"
                "    https://github.com/paydaycoin/bips/blob/master/bip-0023.mediawiki\n"
                "    https://github.com/paydaycoin/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
                "    https://github.com/paydaycoin/bips/blob/master/bip-0145.mediawiki\n",
                {
    ...
    if(memInfo.MemTotalMiB < 8000)
        throw JSONRPCError(RPC_MISC_ERROR, "Insufficient memory, you need at least 8GB RAM to mine paydaycoin and be running in a Unix OS. Please see documentation.");
    LogPrintf("Total number of physical cores found %d\n", GetNumCores());
    if(GetNumCores() < 4)
        throw JSONRPCError(RPC_MISC_ERROR, "Insufficient CPU cores, you need at least 4 cores to mine paydaycoin. Please see documentation.");
    LOCK(cs_main);
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(::ChainActive().Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PaydayCoin Core is downloading masternode winners...");
  
    // next bock is a superblock and we need governance info to correctly construct it
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(::ChainActive().Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PaydayCoin Core is syncing with network...");
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            RPCHelpMan{"submitblock",
                ...
                "See https://en.paydaycoin.it/wiki/BIP_0022 for full specification.\n",
                {
