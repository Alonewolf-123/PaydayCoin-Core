    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"addnode",
                ...
                RPCResults{},
                RPCExamples{
                    HelpExampleCli("addnode", "\"192.168.0.6:8123\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:8123\", \"onetry\"")
                },
            }.ToString());
    ...
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            RPCHelpMan{"disconnectnode",
                ...
                RPCResults{},
                RPCExamples{
                    HelpExampleCli("disconnectnode", "\"192.168.0.6:8123\"")
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:8123\"")
...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8123\",  (string) The paydaycoin server IP and port we're connected to\n"
