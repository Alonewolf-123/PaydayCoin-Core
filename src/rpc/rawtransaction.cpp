...
{
    // Call into TxToUniv() in paydaycoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in paydaycoin-common, so we query them here and push the
...
static UniValue getrawtransaction(const JSONRPCRequest& request)
{
    const RPCHelpMan help{
            ...
            "           \"address\"        (string) paydaycoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) paydaycoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
	...
	if (!CheckPaydayCoinLockedOutpoints(tx, state))
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded partially signed PaydayCoin transaction.\n",
                {
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) PaydayCoin address if there is one\n"
            "        }\n"
            "      },\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed PaydayCoin transactions into one transaction.\n"
