...
CMasternode::CMasternode() : masternode_info_t{MASTERNODE_PRE_ENABLED, MIN_PEER_PROTO_VERSION, GetAdjustedTime(), 0}
{
}

CMasternode::CMasternode(CService addr, COutPoint outpoint, CPubKey pubKeyCollateralAddress, CPubKey pubKeyMasternode, int nProtocolVersionIn, int retries, int nLevelMN) : masternode_info_t{MASTERNODE_PRE_ENABLED, nProtocolVersionIn, GetAdjustedTime(),
                                                                                                                                                                  outpoint, addr, pubKeyCollateralAddress, pubKeyMasternode, retries, nLevelMN}
{
    nLevel = nLevelMN;
}
...
CMasternode::CMasternode(const CMasternodeBroadcast& mnb) : masternode_info_t{mnb.nActiveState, mnb.nProtocolVersion, mnb.sigTime,
                                                                mnb.outpoint, mnb.addr, mnb.pubKeyCollateralAddress, mnb.pubKeyMasternode, mnb.nPingRetries, mnb.nLevel},
...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CPubKey& pubkey, int& nHeightRet)
{
    ...
    if (coin.out.nValue != Params().MasternodeCollateralLevel1() && coin.out.nValue != Params().MasternodeCollateralLevel2()
     && coin.out.nValue != Params().MasternodeCollateralLevel3() && coin.out.nValue != Params().MasternodeCollateralLevel4())
    {
...
void CMasternode::Check(bool fForce)
{
    ...
    if (!fWaitForPing || fOurMasternode) {
        ...
        // part 1: expire based on paydaycoind ping
...
std::string CMasternode::GetLevel(int nLevelMN)
{
    switch (nLevelMN) {
    case MN_Level1:
        return "Level 1";
...
void CMasternode::UpdateLastPaid(const CBlockIndex* pindex, int nMaxBlocksToScanBack)
{
    ...
    for (int i = 0; BlockReading && BlockReading->nHeight > nBlockLastPaid && i < nMaxBlocksToScanBack; i++) {
        if (mnpayments.mapMasternodeBlocks.count(BlockReading->nHeight) &&
            ...
            const CAmount& nMasternodePayment = GetBlockSubsidy(BlockReading->nHeight, chainparams.GetConsensus(), nTotal, false, true, payee.nStartHeight) * 0.05;
...
int GetMasterNodeLevel(const COutPoint& outpoint, const CPubKey& pubkey) {
    AssertLockHeld(cs_main);
    ...
    if (coin.out.nValue == Params().MasternodeCollateralLevel1())
    {
        return MN_Level1;
    } 
    if (coin.out.nValue == Params().MasternodeCollateralLevel2()) {
        return MN_Level2;
    }
    if (coin.out.nValue == Params().MasternodeCollateralLevel3())
    {
        return MN_Level3;
    } 
    if (coin.out.nValue == Params().MasternodeCollateralLevel4()) {
        return MN_Level4;
    }
...
bool CMasternodeBroadcast::Create(interfaces::Wallet& wallet, const std::string& strService, const std::string& strKeyMasternode, const std::string& strTxHash, const std::string& strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOffline)
{
    ...
    int nLevelMN = GetMasterNodeLevel(outpoint, pubKeyCollateralAddressNew);
    ...
    return Create(outpoint, service, keyCollateralAddressNew, pubKeyCollateralAddressNew, keyMasternodeNew, pubKeyMasternodeNew, strErrorRet, mnbRet, nLevelMN);
}
...
bool CMasternodeBroadcast::Create(const COutPoint& outpoint, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, int nLevelMN)
{
    // wait for reindex and/or import to finish
    if (fImporting || fReindex) return false;
    ...
    mnbRet = CMasternodeBroadcast(service, outpoint, pubKeyCollateralAddressNew, pubKeyMasternodeNew, MIN_PEER_PROTO_VERSION, nLevelMN);
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint(BCLog::MN, "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 100000 PDAY, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
