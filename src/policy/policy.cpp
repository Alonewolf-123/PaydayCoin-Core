...
bool IsStandardTx(const CTransaction& tx, bool permit_bare_multisig, const CFeeRate& dust_relay_fee, std::string& reason)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (whichType == TX_NULL_DATA){
            // PAYDAYCOIN if not paydaycoin tx and opreturn size is bigger than maxcarrier bytes, return false
            // we need this because if it is a sys tx then we allow 200x maxcarrier bytes.
            if (!IsPaydayCoinTx(tx.nVersion) && txout.scriptPubKey.size() > nMaxDatacarrierBytes)
            {
                reason = "scriptpubkey";
                return false;
            }
