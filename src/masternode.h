...
#ifndef PAYDAYCOIN_MASTERNODE_H
#define PAYDAYCOIN_MASTERNODE_H
...
struct masternode_info_t {
    ...
    masternode_info_t(int activeState, int protoVer, int64_t sTime, int retries) : nActiveState{activeState}, nProtocolVersion{protoVer}, sigTime{sTime}, nPingRetries{retries} {}

    masternode_info_t(int activeState, int protoVer, int64_t sTime, COutPoint const& outpnt, CService const& addr, CPubKey const& pkCollAddr, CPubKey const& pkMN, int retries, int nLevelMN) : nActiveState{activeState}, nProtocolVersion{protoVer}, sigTime{sTime},
                                                                                                                                                                                                outpoint{outpnt}, addr{addr},
                                                                                                                                                                                                pubKeyCollateralAddress{pkCollAddr}, pubKeyMasternode{pkMN}, nPingRetries{retries} { nLevel = nLevelMN; }
...
//
// The Masternode Class. It contains the input of the 100000PDAY, signature to prove
...
public:
    ...
    CMasternode(CService addrNew, COutPoint outpointNew, CPubKey pubKeyCollateralAddressNew, CPubKey pubKeyMasternodeNew, int nProtocolVersionIn, int retries, int nLevelMN);
    ...
    static std::string GetLevel(int nLevelMN);
...
public:
    ...
    CMasternodeBroadcast(const CMasternode& mn) : CMasternode(mn), fRecovery(false) {nLevel = mn.nLevel;}
    CMasternodeBroadcast(CService addrNew, COutPoint outpointNew, CPubKey pubKeyCollateralAddressNew, CPubKey pubKeyMasternodeNew, int nProtocolVersionIn, int nLevelMN) : CMasternode(addrNew, outpointNew, pubKeyCollateralAddressNew, pubKeyMasternodeNew, nProtocolVersionIn, 0, nLevelMN), fRecovery(false) {nLevel = nLevelMN;}
    ...
    static bool Create(const COutPoint& outpoint, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, int nLevelMN);
