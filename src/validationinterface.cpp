...
struct ValidationInterfaceConnections {
    ...
    boost::signals2::scoped_connection NotifyPaydayCoinUpdate;
...
struct MainSignalsInstance {
    ...
    boost::signals2::signal<void(const char *value, const char *topic)> NotifyPaydayCoinUpdate;
...
void RegisterValidationInterface(CValidationInterface* pwalletIn) {
    ...
    // PAYDAYCOIN
    conns.NotifyPaydayCoinUpdate = g_signals.m_internals->NotifyPaydayCoinUpdate.connect(std::bind(&CValidationInterface::NotifyPaydayCoinUpdate, pwalletIn, std::placeholders::_1, std::placeholders::_2));
...
// PAYDAYCOIN
void CMainSignals::NotifyPaydayCoinUpdate(const char *value, const char *topic) {
    m_internals->NotifyPaydayCoinUpdate(value, topic);
}
void CMainSignals::NotifyHeaderTip(const CBlockIndex * pindex, bool fInitialDownload) {
    m_internals->NotifyHeaderTip(pindex, fInitialDownload);
}
void CMainSignals::AcceptedBlockHeader(const CBlockIndex * pindex) {
    m_internals->AcceptedBlockHeader(pindex);
}