...
UniValue paydaycointxfund(const JSONRPCRequest& request);
...
UniValue paydaycointxfund_helper(const string& strAddress, const int &nVersion, const string &vchWitness, const vector<CRecipient> &vecSend, const COutPoint& outpoint=emptyOutPoint) {
    ...
    if (!vchWitness.empty() && vchWitness != "''")
    {
        ...
        {
            throw runtime_error("PAYDAYCOIN_RPC_ERROR ERRCODE: 9000 - " + _("This transaction requires a witness but no enough outputs found for witness address: ") + strWitnessAddress);
        }
        Coin pcoinW;
    ...
    {
        throw runtime_error("PAYDAYCOIN_RPC_ERROR ERRCODE: 9000 - " + _("Not enough outputs found for address: ") + strAddress);
    }
    Coin pcoin;
    if (GetUTXOCoin(addressOutpoint, pcoin)){
        ...
        if(fTPSTest && fTPSTestEnabled){
            if(std::find(savedtxins.begin(), savedtxins.end(), txIn) == savedtxins.end()){
                ...
                LogPrint(BCLog::PDAY, "Skipping saved output in paydaycointxfund_helper...\n");
            }
        }
        else
            txNew.vin.push_back(txIn);
    ...
    return paydaycointxfund(request);
}
...
UniValue paydaycointxfund(const JSONRPCRequest& request) {
    ...
    if (request.fHelp || 1 > params.size() || 3 < params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycointxfund",
                "\nFunds a new paydaycoin transaction with inputs used from wallet or an array of addresses specified. Note that any inputs to the transaction added prior to calling this will not be accounted and new outputs will be added every time you call this function.\n",
                {
                    {"hexstring", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The raw paydaycoin transaction output given from rpc"},
                ...
                RPCExamples{
                    HelpExampleCli("paydaycointxfund", "<hexstring> \"sys1qtyf33aa2tl62xhrzhralpytka0krxvt0a4e8ee\"")
                    + HelpExampleRpc("paydaycointxfund", "<hexstring>, \"sys1qtyf33aa2tl62xhrzhralpytka0krxvt0a4e8ee\", 0")
    ...
    if (!DecodeHexTx(txIn, hexstring, true, false))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 5500 - " + _("Could not send raw transaction: Cannot decode transaction from hex string: ") + hexstring);
    ...
    if(txIn_t.nVersion == PAYDAYCOIN_TX_VERSION_MINT) 
        nDesiredAmount = 0;
    ...
    if (resUTXOs.isObject()) {
        ...
        if (!resUtxoUnspents.isArray())
            throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 5501 - " + _("No unspent outputs found in addresses provided"));
        utxoArray = resUtxoUnspents.get_array();
    }
    else
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 5501 - " + _("No funds found in addresses provided"));
        ...
        for (unsigned int i = 0; i < utxoArray.size(); i++)
        {
            ...
            if(fTPSTest && fTPSTestEnabled){
                if(std::find(savedtxins.begin(), savedtxins.end(), txIn) == savedtxins.end())
                    ...
                    LogPrint(BCLog::PDAY, "Skipping saved output in paydaycointxfund...\n");
                    continue;
                }
            }
            int numSigs = 0;
    ...
    if (nChange < 0)
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 5502 - " + _("Insufficient funds, short by: ") + ValueFromAmount(-1*nChange).write() + " PDAY");
...
UniValue paydaycoinburn(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 3 != params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycoinburn",
                "\nBurns the paydaycoin for bridging to Ethereum token\n",
                {
                    {"funding_address", RPCArg::Type::STR, RPCArg::Optional::NO, "Funding address to burn PDAY from"},
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "Amount of PDAY to burn. Note that fees are applied on top. It is not inclusive of fees."},
                ...
                RPCExamples{
                    HelpExampleCli("paydaycoinburn", "\"funding_address\" \"amount\" \"ethaddress\"")
                    + HelpExampleRpc("paydaycoinburn", "\"funding_address\", \"amount\", \"ethaddress\"")
                }
         }.ToString());
    ...
    UniValue res = paydaycointxfund_helper(fundingAddress, ethAddress.empty()? 0: PAYDAYCOIN_TX_VERSION_BURN, "", vecSend);
    return res;
}
UniValue paydaycoinmint(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 11 != params.size())
        throw runtime_error(
                RPCHelpMan{"paydaycoinmint",
                "\nMint paydaycoin to come back from the ethereum bridge\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "Mint to this address."},
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "Amount of PDAY to mint.  Note that fees are applied on top.  It is not inclusive of fees"},
                ...
                RPCExamples{
                    HelpExampleCli("paydaycoinmint","\"address\" \"amount\" \"blocknumber\" \"tx_hex\" \"txroot_hex\" \"txmerkleproof\" \"txmerkleproofpath\" \"receipt_hex\" \"receiptroot_hex\" \"receiptmerkleproof\"")
                    + HelpExampleRpc("paydaycoinmint","\"address\", \"amount\", \"blocknumber\", \"tx_hex\", \"txroot_hex\", \"txmerkleproof\", \"txmerkleproofpath\", \"receipt_hex\", \"receiptroot_hex\", \"receiptmerkleproof\", \"\"")
                }
                }.ToString());
    ...
    if(!fGethSynced){
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 5502 - " + _("Geth is not synced, please wait until it syncs up and try again"));
    }
    int nBlocksLeftToEnable = ::ChainActive().Tip()->nHeight - (Params().GetConsensus().nBridgeStartBlock+500);
    if(nBlocksLeftToEnable > 0)
    {
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 5502 - " + _("Bridge is not enabled yet. Blocks left to enable: ") + itostr(nBlocksLeftToEnable));
    }
    CMintPaydayCoin mintPaydayCoin;
    mintPaydayCoin.nBlockNumber = nBlockNumber;
    mintPaydayCoin.vchTxValue = ParseHex(vchTxValue);
    mintPaydayCoin.vchTxRoot = ParseHex(vchTxRoot);
    mintPaydayCoin.vchTxParentNodes = ParseHex(vchTxParentNodes);
    mintPaydayCoin.vchTxPath = ParseHex(vchTxPath);
    mintPaydayCoin.vchReceiptValue = ParseHex(vchReceiptValue);
    mintPaydayCoin.vchReceiptRoot = ParseHex(vchReceiptRoot);
    mintPaydayCoin.vchReceiptParentNodes = ParseHex(vchReceiptParentNodes);
    ...
    if(!ethTxRootShouldExist){
        throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2501 - " + _("Network is not ready to accept your mint transaction please wait..."));
    }
    ...
    if(!pethereumtxrootsdb || !pethereumtxrootsdb->ReadTxRoots(mintPaydayCoin.nBlockNumber, txRootDB)){
        if(ethTxRootShouldExist){
            throw runtime_error("PAYDAYCOIN_CONSENSUS_ERROR ERRCODE: 1001 - " + _("Missing transaction root for SPV proof at Ethereum block: ") + itostr(mintPaydayCoin.nBlockNumber));
        }
    }  
    if(ethTxRootShouldExist){
        ...
        if(fGethSyncHeight >= MAX_ETHEREUM_TX_ROOTS && mintPaydayCoin.nBlockNumber <= (uint32_t)cutoffHeight) {
            throw runtime_error("PAYDAYCOIN_CONSENSUS_ERROR ERRCODE: 1001 - " + _("The block height is too old, your SPV proof is invalid. SPV Proof must be done within 40000 blocks of the burn transaction on Ethereum blockchain"));
        } 
        ...
        if(fGethCurrentHeight <  mintPaydayCoin.nBlockNumber || fGethSyncHeight <= 0 || (fGethSyncHeight - mintPaydayCoin.nBlockNumber < (bGethTestnet? 20: ETHEREUM_CONFIRMS_REQUIRED*1.5))){
            throw runtime_error("PAYDAYCOIN_CONSENSUS_ERROR ERRCODE: 1001 - " + _("Not enough confirmations on Ethereum to process this mint transaction. Blocks required: ") + itostr((ETHEREUM_CONFIRMS_REQUIRED*1.5) - (fGethSyncHeight - mintPaydayCoin.nBlockNumber)));
        } 
    }
          
    vector<unsigned char> data;
    mintPaydayCoin.Serialize(data);
    ...
    UniValue res = paydaycointxfund_helper(vchAddress, PAYDAYCOIN_TX_VERSION_MINT, strWitnessAddress, vecSend);
    return res;
}
...
UniValue assetnew(const JSONRPCRequest& request) {
    ...
    if(find_value(detail.get_obj(), "iswitness").get_bool() == false)
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2501 - " + _("Address must be a segwit based address"));
    ...
    // build asset object
    CAsset newAsset;
    newAsset.nAsset = GeneratePaydayCoinGuid();
    ...
    CreateFeeRecipient(scriptData, fee);
    vecSend.push_back(fee);
    UniValue res = paydaycointxfund_helper(vchAddress, PAYDAYCOIN_TX_VERSION_ASSET_ACTIVATE, vchWitness, vecSend);
...
UniValue assetupdate(const JSONRPCRequest& request) {
        ...
        throw runtime_error(
            RPCHelpMan{"assetupdate",
                "\nPerform an update on an asset you control.\n",
                {
                    ...
                    {"public_value", RPCArg::Type::STR, RPCArg::Optional::NO, "Public data, 256 characters max."},
                    {"contract",  RPCArg::Type::STR, RPCArg::Optional::NO, "Ethereum token contract for PaydayCoinX bridge. Leave empty for no smart contract bridg."},
    ...
    if (!GetAsset( nAsset, theAsset))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2501 - " + _("Could not find a asset with this key"));
    ...
    CreateFeeRecipient(scriptData, fee);
    vecSend.push_back(fee);
    return paydaycointxfund_helper(theAsset.witnessAddress.ToString(), PAYDAYCOIN_TX_VERSION_ASSET_UPDATE, vchWitness, vecSend);
}
...
UniValue assettransfer(const JSONRPCRequest& request) {
    ...
    CAsset theAsset;
    if (!GetAsset( nAsset, theAsset))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2505 - " + _("Could not find a asset with this key"));
    ...
    CreateFeeRecipient(scriptData, fee);
    vecSend.push_back(fee);
    return paydaycointxfund_helper(theAsset.witnessAddress.ToString(), PAYDAYCOIN_TX_VERSION_ASSET_TRANSFER, vchWitness, vecSend);
}
UniValue assetsendmany(const JSONRPCRequest& request) {
    ...
    CAsset theAsset;
    if (!GetAsset(nAsset, theAsset))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2507 - " + _("Could not find a asset with this key"));
    ...
    for (unsigned int idx = 0; idx < receivers.size(); idx++) {
        ...
        if(toStr != "burn"){
            ...
            if(!IsValidDestination(dest))
                throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2509 - " + _("Asset must be sent to a valid paydaycoin address"));
    ...
    return paydaycointxfund_helper(theAsset.witnessAddress.ToString(), PAYDAYCOIN_TX_VERSION_ASSET_SEND, vchWitness, vecSend);
}
...
UniValue assetsend(const JSONRPCRequest& request) {
	...
	CAsset theAsset;
	if (!GetAsset(nAsset, theAsset))
		throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("Could not find a asset with this key"));            
...
UniValue assetallocationsendmany(const JSONRPCRequest& request) {
    ...
    if(strAddress != "burn"){
    	...
    	if (IsValidDestination(addressFrom)) {
            ...
            if(find_value(detail.get_obj(), "iswitness").get_bool() == false)
                throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2501 - " + _("Address must be a segwit based address"));
	...
	if (!GetAssetAllocation(assetAllocationTuple, theAssetAllocation))
		throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1500 - " + _("Could not find a asset allocation with this key"));
	...
	for (unsigned int idx = 0; idx < receivers.size(); idx++) {
        ...
        if(toStr != "burn"){
            CTxDestination dest = DecodeDestination(toStr);
            if(!IsValidDestination(dest))
                throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 2509 - " + _("Asset must be sent to a valid paydaycoin address"));
    ...
    
    {
    	...
    	for (auto& arrivalTime : arrivalTimes) {
    		...
    		if ((nNow - arrivalTime.second) < minLatency) {
    			throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1503 - " + _("Please wait a few more seconds and try again..."));
    		}
    	}
    }
	...
	CreateFeeRecipient(scriptData, fee);
	vecSend.push_back(fee);
	return paydaycointxfund_helper(strAddressFrom, PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_SEND, strWitness, vecSend, lockedOutpoint);
}
template <typename T>
...
UniValue assetallocationburn(const JSONRPCRequest& request) {
		...
		throw runtime_error(
            RPCHelpMan{"assetallocationburn",
                "\nBurn an asset allocation in order to use the bridge\n",
                {
                    ...
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "Amount of asset to burn to PDAYX"},
	...
	return paydaycointxfund_helper(strAddress, PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_BURN, "", vecSend);
}
UniValue assetallocationmint(const JSONRPCRequest& request) {
    ...
    CMintPaydayCoin mintPaydayCoin;
    mintPaydayCoin.assetAllocationTuple = CAssetAllocationTuple(nAsset, CWitnessAddress(witnessVersion, ParseHex(witnessProgramHex)));
    mintPaydayCoin.nValueAsset = nAmount;
    mintPaydayCoin.nBlockNumber = nBlockNumber;
    mintPaydayCoin.vchTxValue = ParseHex(vchTxValue);
    mintPaydayCoin.vchTxRoot = ParseHex(vchTxRoot);
    mintPaydayCoin.vchTxParentNodes = ParseHex(vchTxParentNodes);
    ...
    return paydaycointxfund_helper(strAddress, PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_MINT, strWitness, vecSend);
}
...
UniValue assetallocationlock(const JSONRPCRequest& request) {
    ...
    if(!IsOutpointMature(theAssetAllocation.lockedOutpoint))
        throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1500 - " + _("Outpoint not mature"));
    Coin pcoin;
    GetUTXOCoin(theAssetAllocation.lockedOutpoint, pcoin);
    CTxDestination address;
    if (!ExtractDestination(pcoin.out.scriptPubKey, address))
        throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1500 - " + _("Could not extract destination from outpoint"));
        
    const string& strAddressDest = EncodeDestination(address);
    if(strAddressFrom != strAddressDest)    
        throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1500 - " + _("Outpoint address must match allocation owner address"));
	...
	return paydaycointxfund_helper(strAddressFrom, PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_LOCK, strWitness, vecSend);
}
UniValue sendfrom(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            RPCHelpMan{"sendfrom",
                ...
                {
                    {"funding_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The paydaycoin address is sending from."},
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The paydaycoin address to send to."},
    ...
    return paydaycointxfund_helper(EncodeDestination(from), 0, strWitness, vecSend);
}
...
{ //  category              name                                actor (function)                argNames
   ...
   /* assets using the blockchain, coins/points/service backed tokens*/
    { "paydaycoinwallet",            "paydaycoinburn",                      &paydaycoinburn,                   {"funding_address","amount","ethereum_destination_address"} },
    { "paydaycoinwallet",            "paydaycoinmint",                      &paydaycoinmint,                   {"address","amount","blocknumber","tx_hex","txroot_hex","txmerkleproof_hex","txmerkleproofpath_hex","receipt_hex","receiptroot_hex","receiptmerkleproof","witness"} }, 
    { "paydaycoinwallet",            "assetallocationburn",              &assetallocationburn,           {"asset_guid","address","amount","ethereum_destination_address"} }, 
    { "paydaycoinwallet",            "assetallocationmint",              &assetallocationmint,           {"asset_guid","address","amount","blocknumber","tx_hex","txroot_hex","txmerkleproof_hex","txmerkleproofpath_hex","receipt_hex","receiptroot_hex","receiptmerkleproof","witness"} },     
    { "paydaycoinwallet",            "paydaycointxfund",                    &paydaycointxfund,                 {"hexstring","address","output_index"}},
    { "paydaycoinwallet",            "assetnew",                         &assetnew,                      {"address","symbol","public value","contract","precision","total_supply","max_supply","update_flags","witness"}},
    { "paydaycoinwallet",            "assetupdate",                      &assetupdate,                   {"asset_guid","public value","contract","supply","update_flags","witness"}},
    { "paydaycoinwallet",            "assettransfer",                    &assettransfer,                 {"asset_guid","address","witness"}},
    { "paydaycoinwallet",            "assetsend",                        &assetsend,                     {"asset_guid","address","amount"}},
    { "paydaycoinwallet",            "assetsendmany",                    &assetsendmany,                 {"asset_guid","inputs"}},
    { "paydaycoinwallet",            "assetallocationlock",              &assetallocationlock,           {"asset_guid","address","txid","output_index","witness"}},
    { "paydaycoinwallet",            "assetallocationsend",              &assetallocationsend,           {"asset_guid","address_sender","address_receiver","amount"}},
    { "paydaycoinwallet",            "assetallocationsendmany",          &assetallocationsendmany,       {"asset_guid","address","inputs","witness"}},
    { "paydaycoinwallet",            "sendfrom",                         &sendfrom,                      {"funding_address","address","amount"}},
};
// clang-format on
