...
// PAYDAYCOIN service rpc functions
...
UniValue convertaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nConvert between PaydayCoin 3 and PaydayCoin 4 formats. P2WPKH can be shown as P2PKH in PaydayCoin 3.\n",
            {
                {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The paydaycoin address to get the information of."}
            },
            RPCResult{
                "{\n"
                "  \"v3address\" : \"address\",        (string) The paydaycoin 3 address validated\n"
                "  \"v4address\" : \"address\",        (string) The paydaycoin 4 address validated\n"
                "}\n"
            },
            RPCExamples{
...
UniValue tpstestinfo(const JSONRPCRequest& request) {
	...
	if(!fTPSTest)
		throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("This function requires tpstest configuration to be set upon startup. Please shutdown and enable it by adding it to your paydaycoin.conf file and then call 'tpstestsetenabled true'."));
...
UniValue tpstestsetenabled(const JSONRPCRequest& request) {
	...
	if(!fTPSTest)
		throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("This function requires tpstest configuration to be set upon startup. Please shutdown and enable it by adding it to your paydaycoin.conf file and then try again."));
...
UniValue tpstestadd(const JSONRPCRequest& request) {
	...
	if (fTPSTestEnabled) {
		...
		if (bFirstTime) {
			...
			if (!isThreadPosted)
				throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1501 - " + _("thread pool queue is full"));
		}
	}
	UniValue result(UniValue::VOBJ);
	result.__pushKV("status", "success");
...
UniValue assetallocationbalance(const JSONRPCRequest& request) {
    ...
    if (passetallocationdb == nullptr || !passetallocationdb->ReadAssetAllocation(assetAllocationTuple, txPos))
        throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1507 - " + _("Failed to read from assetallocation DB"));

    CAsset theAsset;
    if (!GetAsset(nAsset, theAsset))
        throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1508 - " + _("Could not find a asset with this key"));
            ...
            RPCHelpMan{"assetallocationbalances",
                "\nShow stored balance of multiple asset allocations.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, RPCArg::Optional::NO, "The addresses owning the allocations",
                                {
                                    {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "paydaycoin address"},
                                },
                    }
                },
                RPCResult{
...
UniValue listassetindexallocations(const JSONRPCRequest& request) {
    ...
    if(!fAssetIndex){
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 1510 - " + _("You must reindex paydaycoin with -assetindex enabled"));
    }       
...
UniValue listassetallocations(const JSONRPCRequest& request) {
	...
	if (params.size() > 0) {
		...
		if (count < 0) {
			throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1510 - " + _("'count' must be 0 or greater"));
		}
	}
	if (params.size() > 1) {
		from = params[1].get_int();
		if (from < 0) {
			throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1510 - " + _("'from' must be 0 or greater"));
		}
	}
	if (params.size() > 2) {
		options = params[2];
	...
	if (!passetallocationdb->ScanAssetAllocations(count, from, options, oRes))
		throw runtime_error("PAYDAYCOIN_ASSET_ALLOCATION_RPC_ERROR: ERRCODE: 1510 - " + _("Scan failed"));
	return oRes;
}
...
UniValue paydaycoindecoderawtransaction(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 1 != params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycoindecoderawtransaction",
            "\nDecode raw paydaycoin transaction (serialized, hex-encoded) and display information pertaining to the service that is included in the transactiion data output(OP_RETURN)\n",
            {
                {"hexstring", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The transaction hex string."}
            },
            RPCResult{
            "{\n"
            "  \"txtype\" : \"txtype\",         (string) The paydaycoin transaction type\n"
            ...
            RPCExamples{
                HelpExampleCli("paydaycoindecoderawtransaction", "\"hexstring\"")
                + HelpExampleRpc("paydaycoindecoderawtransaction", "\"hexstring\"")
            }
            }.ToString());
    ...
    if (rawTx.IsNull())
        throw runtime_error("PAYDAYCOIN_RPC_ERROR: ERRCODE: 5512 - " + _("Could not decode transaction"));
    
    UniValue output(UniValue::VOBJ);
    if(!DecodePaydayCoinRawtransaction(rawTx, output))
        throw runtime_error("PAYDAYCOIN_RPC_ERROR: ERRCODE: 5512 - " + _("Not a PaydayCoin transaction"));
    return output;
}
...
UniValue addressbalance(const JSONRPCRequest& request) {
    ...
    if (request.fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nShow the PaydayCoin balance of an address\n",
            {
                {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "Address to holding the balance"}
            },
            RPCResult{
            "{\n"
            "  \"amount\": xx            (numeric) PaydayCoin balance of the address\n"
            "}\n"
            },
            RPCExamples{
...
UniValue assetinfo(const JSONRPCRequest& request) {
    ...
    if (passetdb == nullptr || !passetdb->ReadAsset(nAsset, txPos))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2511 - " + _("Failed to read from asset DB"));
...
UniValue listassets(const JSONRPCRequest& request) {
    ...
    if (params.size() > 0) {
        ...
        if (count < 0) {
            throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("'count' must be 0 or greater"));
        }
    }
    if (params.size() > 1) {
        from = params[1].get_int();
        if (from < 0) {
            throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("'from' must be 0 or greater"));
        }
    }
    if (params.size() > 2) {
        options = params[2];
    ...
    if (!passetdb->ScanAssets(count, from, options, oRes))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("Scan failed"));
    return oRes;
}
...
UniValue paydaycoingetspvproof(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"paydaycoingetspvproof",
            ...
            RPCExamples{
                HelpExampleCli("paydaycoingetspvproof", "dfc7eac24fa89b0226c64885f7bedaf132fc38e8980b5d446d76707027254490")
                + HelpExampleRpc("paydaycoingetspvproof", "dfc7eac24fa89b0226c64885f7bedaf132fc38e8980b5d446d76707027254490")
            }
            }.ToString());
    LOCK(cs_main);
    UniValue res(UniValue::VOBJ);
    ...
    try{
        ...
        UniValue resDecode = paydaycoindecoderawtransaction(requestDecodeRPC);
    ...
    if(!assetVal.isNull()) {
        ...
        if (!GetAsset(assetVal.get_int(), asset))
             throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 1510 - " + _("Asset not found"));
        if(asset.vchContract.empty())
            throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 1510 - " + _("Asset contract is empty"));
         res.__pushKV("contract", HexStr(asset.vchContract));    
                   
    }
    else{
        res.__pushKV("contract", HexStr(Params().GetConsensus().vchPDAYXContract));
    }
    return res;
}
UniValue listassetindex(const JSONRPCRequest& request) {
    ...
    if(!fAssetIndex){
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 1510 - " + _("You must start paydaycoin with -assetindex enabled"));
    }
    UniValue options;
    int64_t page = params[0].get_int64();
   
    if (page < 0) {
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 1510 - " + _("'page' must be 0 or greater"));
    }
    ...
    if (!passetindexdb->ScanAssetIndex(page, options, oRes))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 1510 - " + _("Scan failed"));
    return oRes;
}
...
UniValue paydaycoinstopgeth(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 0 != params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycoinstopgeth",
            ...
            RPCExamples{
                HelpExampleCli("paydaycoinstopgeth", "")
                + HelpExampleRpc("paydaycoinstopgeth", "")
            }
            }.ToString());
    if(!StopRelayerNode(relayerPID))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("Could not stop relayer"));
    if(!StopGethNode(gethPID))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("Could not stop Geth"));
    UniValue ret(UniValue::VOBJ);
    ret.__pushKV("status", "success");
    return ret;
}
UniValue paydaycoinstartgeth(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 0 != params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycoinstartgeth",
            ...
            RPCExamples{
                HelpExampleCli("paydaycoinstartgeth", "")
                + HelpExampleRpc("paydaycoinstartgeth", "")
            }
            }.ToString());
    
    StopRelayerNode(relayerPID);
    StopGethNode(gethPID);
    int wsport = gArgs.GetArg("-gethwebsocketport", 8717);
    bool bGethTestnet = gArgs.GetBoolArg("-gethtestnet", false);
    if(!StartGethNode(exePath, gethPID, bGethTestnet, wsport))
        throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("Could not start Geth"));
...
UniValue paydaycoinsetethstatus(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 2 != params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycoinsetethstatus",
                ...
                RPCExamples{
                    HelpExampleCli("paydaycoinsetethstatus", "\"syncing\" 7000000")
                    + HelpExampleCli("paydaycoinsetethstatus", "\"synced\" 0")
                    + HelpExampleRpc("paydaycoinsetethstatus", "\"syncing\", 7000000")
                    + HelpExampleRpc("paydaycoinsetethstatus", "\"synced\", 0")
                }
                }.ToString());
    ...
    LogPrint(BCLog::PDAY, "paydaycoinsetethstatus old height %d new height %d\n", nGethOldHeight, fGethCurrentHeight);
    ret.__pushKV("missing_blocks", retArray);
    if(fZMQEthStatus){
        ...
        GetMainSignals().NotifyPaydayCoinUpdate(oEthStatus.write().c_str(), "ethstatus");
    }    
    return ret;
}
UniValue paydaycoinsetethheaders(const JSONRPCRequest& request) {
    const UniValue &params = request.params;
    if (request.fHelp || 1 != params.size())
        throw runtime_error(
            RPCHelpMan{"paydaycoinsetethheaders",
                "\nSets Ethereum headers in PaydayCoin to validate transactions through the PDAYX bridge.\n",
                {
                ...
                RPCExamples{
                    HelpExampleCli("paydaycoinsetethheaders", "\"[[7043888,\\\"0xd8ac75c7b4084c85a89d6e28219ff162661efb8b794d4b66e6e9ea52b4139b10\\\",\\\"0xd8ac75c7b4084c85a89d6e28219ff162661efb8b794d4b66e6e9ea52b4139b10\\\",\\\"0xd8ac75c7b4084c85a89d6e28219ff162661efb8b794d4b66e6e9ea52b4139b10\\\"],...]\"")
                    + HelpExampleRpc("paydaycoinsetethheaders", "\"[[7043888,\\\"0xd8ac75c7b4084c85a89d6e28219ff162661efb8b794d4b66e6e9ea52b4139b10\\\",\\\"0xd8ac75c7b4084c85a89d6e28219ff162661efb8b794d4b66e6e9ea52b4139b10\\\",\\\"0xd8ac75c7b4084c85a89d6e28219ff162661efb8b794d4b66e6e9ea52b4139b10\\\"],...]\"")
                }
            }.ToString());  
    ...
    for(size_t i =0;i<headerArray.size();i++){
        ...
        if(tupleArray.size() != 5)
            throw runtime_error("PAYDAYCOIN_ASSET_RPC_ERROR: ERRCODE: 2512 - " + _("Invalid size in a blocknumber/txroots input, should be size of 5"));
...
UniValue paydaycoingettxroots(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"paydaycoingettxroot",
            ...
            RPCExamples{
                HelpExampleCli("paydaycoingettxroots", "23232322")
                + HelpExampleRpc("paydaycoingettxroots", "23232322")
            }
            }.ToString());
    }
...
{ //  category              name                                actor (function)                argNames
    //  --------------------- ------------------------          -----------------------         ----------
    { "paydaycoin",            "paydaycoingettxroots",                &paydaycoingettxroots,             {"height"} },
    { "paydaycoin",            "getblockhashbytxid",               &getblockhashbytxid,            {"txid"} },
    { "paydaycoin",            "paydaycoingetspvproof",               &paydaycoingetspvproof,            {"txid","blockhash"} },
    { "paydaycoin",            "convertaddress",                   &convertaddress,                {"address"} },
    { "paydaycoin",            "paydaycoindecoderawtransaction",      &paydaycoindecoderawtransaction,   {}},
    { "paydaycoin",            "addressbalance",                   &addressbalance,                {}},
    { "paydaycoin",            "assetinfo",                        &assetinfo,                     {"asset_guid"}},
    { "paydaycoin",            "listassets",                       &listassets,                    {"count","from","options"} },
    { "paydaycoin",            "assetallocationinfo",              &assetallocationinfo,           {"asset_guid"}},
    { "paydaycoin",            "assetallocationbalance",           &assetallocationbalance,        {"asset_guid"}},
    { "paydaycoin",            "assetallocationbalances",          &assetallocationbalances,       {"asset_guid","addresses"} },
    { "paydaycoin",            "assetallocationsenderstatus",      &assetallocationsenderstatus,   {"asset_guid"}},
    { "paydaycoin",            "listassetallocations",             &listassetallocations,          {"count","from","options"} },
    { "paydaycoin",            "listassetallocationmempoolbalances",             &listassetallocationmempoolbalances,          {"count","from","options"} },
    { "paydaycoin",            "listassetindex",                   &listassetindex,                {"page","options"} },
    { "paydaycoin",            "listassetindexassets",             &listassetindexassets,          {"address"} },
    { "paydaycoin",            "listassetindexallocations",        &listassetindexallocations,     {"address"} },
    { "paydaycoin",            "tpstestinfo",                      &tpstestinfo,                   {} },
    { "paydaycoin",            "tpstestadd",                       &tpstestadd,                    {"starttime","rawtxs"} },
    { "paydaycoin",            "tpstestsetenabled",                &tpstestsetenabled,             {"enabled"} },
    { "paydaycoin",            "paydaycoinsetethstatus",              &paydaycoinsetethstatus,           {"syncing_status","highestBlock"} },
    { "paydaycoin",            "paydaycoinsetethheaders",             &paydaycoinsetethheaders,          {"headers"} },
    { "paydaycoin",            "paydaycoinstopgeth",                  &paydaycoinstopgeth,               {} },
    { "paydaycoin",            "paydaycoinstartgeth",                 &paydaycoinstartgeth,              {} },
};
// clang-format on
