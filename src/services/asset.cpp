...
int GetPaydayCoinDataOutput(const CTransaction& tx) {
...
bool GetPaydayCoinData(const CTransaction &tx, vector<unsigned char> &vchData, int& nOut)
{
	nOut = GetPaydayCoinDataOutput(tx);
	if (nOut == -1)
		return false;

	const CScript &scriptPubKey = tx.vout[nOut].scriptPubKey;
	return GetPaydayCoinData(scriptPubKey, vchData);
}
bool GetPaydayCoinData(const CScript &scriptPubKey, vector<unsigned char> &vchData)
{
...
bool GetPaydayCoinBurnData(const CTransaction &tx, CAssetAllocation* theAssetAllocation, std::vector<unsigned char> &vchEthAddress)
{   
...
bool GetPaydayCoinBurnData(const CTransaction &tx, uint32_t& nAssetFromScript, CWitnessAddress& burnWitnessAddress, CAmount &nAmountFromScript, std::vector<unsigned char> &vchEthAddress)
{
    if(tx.nVersion != PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_BURN){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Invalid transaction version\n");
        return false;
    }
    int nOut = GetPaydayCoinDataOutput(tx);
    if (nOut == -1){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Data index must be positive\n");
        return false;
    }

    const CScript &scriptPubKey = tx.vout[nOut].scriptPubKey;
    std::vector<std::vector< unsigned char> > vvchArgs;
    if(!GetPaydayCoinBurnData(scriptPubKey, vvchArgs)){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Cannot get burn data\n");
        return false;
    }
        
    if(vvchArgs.size() != 5){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Wrong argument size %d\n", vvchArgs.size());
        return false;
    }
          
    if(vvchArgs[0].size() != 4){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: nAssetFromScript - Wrong argument size %d\n", vvchArgs[0].size());
        return false;
    }
    ...
    if(vvchArgs[1].size() != 8){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: nAmountFromScript - Wrong argument size %d\n", vvchArgs[1].size());
        return false; 
    }
    ...
    if(vvchArgs[2].empty()){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Ethereum address empty\n");
        return false; 
    }
    vchEthAddress = vvchArgs[2]; 
    if(vvchArgs[3].size() != 1){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Witness address version - Wrong argument size %d\n", vvchArgs[3].size());
        return false;
    }
    const unsigned char &nWitnessVersion = static_cast<unsigned char>(vvchArgs[3][0]);
    
    if(vvchArgs[4].empty()){
        LogPrint(BCLog::PDAY, "GetPaydayCoinBurnData: Witness address empty\n");
        return false;
    }     
...
bool GetPaydayCoinBurnData(const CScript &scriptPubKey, std::vector<std::vector<unsigned char> > &vchData)
{
    CScript::const_iterator pc = scriptPubKey.begin();
    opcodetype opcode;
    if (!scriptPubKey.GetOp(pc, opcode))
...
bool CMintPaydayCoin::UnserializeFromData(const vector<unsigned char> &vchData) {
    try {
...
bool CAsset::UnserializeFromTx(const CTransaction &tx) {
	...
	int nOut;
	if (!IsAssetTx(tx.nVersion) || !GetPaydayCoinData(tx, vchData, nOut))
	{
		SetNull();
		return false;
	}
...
bool CMintPaydayCoin::UnserializeFromTx(const CTransaction &tx) {
    vector<unsigned char> vchData;
    int nOut;
    if (!IsPaydayCoinMintTx(tx.nVersion) || !GetPaydayCoinData(tx, vchData, nOut))
    {
        SetNull();
        return false;
    }
...
void CTxMemPool::removeExpiredMempoolBalances(setEntries& stage){ 
    ...
    for (const txiter& it : stage) {
        ...
        if(IsAssetAllocationTx(tx.nVersion) && tx.nVersion != PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_LOCK){
...
bool IsPaydayCoinMintTx(const int &nVersion){
    return nVersion == PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_MINT || nVersion == PAYDAYCOIN_TX_VERSION_MINT;
}
bool IsAssetTx(const int &nVersion){
    return nVersion == PAYDAYCOIN_TX_VERSION_ASSET_ACTIVATE || nVersion == PAYDAYCOIN_TX_VERSION_ASSET_UPDATE || nVersion == PAYDAYCOIN_TX_VERSION_ASSET_TRANSFER || nVersion == PAYDAYCOIN_TX_VERSION_ASSET_SEND;
}
bool IsAssetAllocationTx(const int &nVersion){
    return nVersion == PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_BURN || 
        nVersion == PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_SEND || nVersion == PAYDAYCOIN_TX_VERSION_ASSET_ALLOCATION_LOCK;
}
bool IsPaydayCoinTx(const int &nVersion){
    return IsAssetTx(nVersion) || IsAssetAllocationTx(nVersion) || IsPaydayCoinMintTx(nVersion);
}
#ifdef ENABLE_WALLET
bool DecodePaydayCoinRawtransaction(const CTransaction& rawTx, UniValue& output, CWallet* const pwallet, const isminefilter* filter_ismine){
    vector<vector<unsigned char> > vvch;
    bool found = false;
    if(IsPaydayCoinMintTx(rawTx.nVersion)){
        found = AssetMintTxToJson(rawTx, output);
    }
    else if (IsAssetTx(rawTx.nVersion) || IsAssetAllocationTx(rawTx.nVersion) || rawTx.nVersion == PAYDAYCOIN_TX_VERSION_BURN){
...
#endif
bool DecodePaydayCoinRawtransaction(const CTransaction& rawTx, UniValue& output){
    vector<vector<unsigned char> > vvch;
    bool found = false;
    if(IsPaydayCoinMintTx(rawTx.nVersion)){
        found = AssetMintTxToJson(rawTx, output);
...
{
    bool found = false;
    if (IsAssetTx(tx.nVersion) && tx.nVersion != PAYDAYCOIN_TX_VERSION_ASSET_SEND)
        found = AssetTxToJSON(tx, output);
    else if(tx.nVersion == PAYDAYCOIN_TX_VERSION_BURN)
        found = SysBurnTxToJSON(tx, output);
    else if (IsAssetAllocationTx(tx.nVersion) || tx.nVersion == PAYDAYCOIN_TX_VERSION_ASSET_SEND)
...
bool SysBurnTxToJSON(const CTransaction &tx, UniValue &entry)
    ...
    int nOut;
    // we can expect a single data output and thus can expect getpaydaycoindata() to pass and give the ethereum address
    if (!GetPaydayCoinData(tx, vchEthAddress, nOut) || vchEthAddress.size() != MAX_GUID_LENGTH) {
        return false;
    }
    int nHeight = 0;
    ...
    entry.__pushKV("txtype", "paydaycoinburn");
...
int GeneratePaydayCoinGuid()
{
    int rand = 0;
    while(rand <= PAYDAYCOIN_TX_VERSION_MINT)
...
void CMintPaydayCoin::Serialize( vector<unsigned char> &vchData) {
...
void WriteAssetIndexTXID(const uint32_t& nAsset, const uint256& txid){
    ...
    if(!passetindexdb->ReadAssetPage(page)){
        ...
        if(!passetindexdb->WriteAssetPage(page))
           LogPrint(BCLog::PDAY, "Failed to write asset page\n");                  
    }
    ...
    if(!passetindexdb->WriteIndexTXIDs(nAsset, page, TXIDS))
        LogPrint(BCLog::PDAY, "Failed to write asset index txids\n");
}
void CAssetDB::WriteAssetIndex(const CTransaction& tx, const CAsset& dbAsset, const int& nHeight, const uint256& blockhash) {
	if (fZMQAsset || fAssetIndex) {
        if(!fAssetIndexGuids.empty() && std::find(fAssetIndexGuids.begin(),fAssetIndexGuids.end(),dbAsset.nAsset) == fAssetIndexGuids.end()){
            LogPrint(BCLog::PDAY, "Asset cannot be indexed because it is not set in -assetindexguids list\n");
            return;
        }
		UniValue oName(UniValue::VOBJ);
        // assetsends write allocation indexes
        if(tx.nVersion != PAYDAYCOIN_TX_VERSION_ASSET_SEND && AssetTxToJSON(tx, nHeight, blockhash, oName)){
            if(fZMQAsset)
                GetMainSignals().NotifyPaydayCoinUpdate(oName.write().c_str(), "assetrecord");
            if(fAssetIndex)
            {
                const uint256& txid = tx.GetHash();
                WriteAssetIndexTXID(dbAsset.nAsset, txid);
                if(!passetindexdb->WritePayload(txid, oName))
                    LogPrint(BCLog::PDAY, "Failed to write asset index payload\n");
            }
        }
	}
}
...
bool CAssetDB::Flush(const AssetMap &mapAssets){
    ...
    LogPrint(BCLog::PDAY, "Flushing %d assets (erased %d, written %d)\n", mapAssets.size(), erase, write);
...
bool CAssetIndexDB::ScanAssetIndex(int64_t page, const UniValue& oOptions, UniValue& oRes) {
    ...
    if (!oOptions.isNull()) {
        ...
        if (assetObj.isNum()) {
            ...
            LogPrint(BCLog::PDAY, "ScanAssetIndex: failed, asset guid is not a number\n");
            return false;
        }
        ...
        LogPrint(BCLog::PDAY, "ScanAssetIndex: failed, options are not found\n");
        return false;
    }
    ...
    if(pageFound < page){
        LogPrint(BCLog::PDAY, "ScanAssetIndex: failed, no entries found in the page table pageFound %d vs %d page\n",pageFound, page);
        return false;
    }
    ...
    if(scanAllocation){
        if(!ReadIndexTXIDs(assetTuple, page, vecTX)){
            LogPrint(BCLog::PDAY, "ScanAssetIndex: failed, cannot read TXIDs of allocation\n");
            return false;
        }
    }
    else{
        if(!ReadIndexTXIDs(nAsset, page, vecTX)){
            LogPrint(BCLog::PDAY, "ScanAssetIndex: failed, cannot read TXIDs of asset\n");
            return false;
        }
    }
...
bool CAssetIndexDB::FlushErase(const std::vector<uint256> &vecTXIDs){
    ...
    LogPrint(BCLog::PDAY, "Flushing %d asset index removals\n", vecTXIDs.size());
    return WriteBatch(batch);
}
