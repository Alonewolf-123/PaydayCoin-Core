...
*
*   Why is this needed?
*   - In PaydayCoin some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount& blockReward, CAmount& fees, CTxOut& txoutMasternodeRet) const
{
    ...
    if (!GetBlockPayee(nBlockHeight, payee, nStartHeightBlock)) {
        ...
        nLevelMN = mnInfo.nLevel;
        nStartHeightBlock = 0;
    } else {
        CMasternodeMan::rank_pair_vec_t mns;
        if (!mnodeman.GetMasternodeRanks(mns, nBlockHeight, GetMinMasternodePaymentsProto())) {
            LogPrint(BCLog::MNPAYMENT, "CMasternodePayments::FillBlockPayee -- nBlockHeight=%d, GetMasternodeRanks failed\n", nBlockHeight);
            return;
        }
        for (const auto& s : mns) {
            CMasternode* mn = mnodeman.Find(s.second.outpoint);
            if (mn != NULL) {
                CScript tmpPayee;
                tmpPayee = GetScriptForDestination(PKHash(mn->pubKeyCollateralAddress));
                if (tmpPayee == payee) {
                    nLevelMN = mn->nLevel;
                    break;
                }
        ...
        if (nLevelMN == -1) {
            LogPrint(BCLog::MNPAYMENT, "CMasternodePayments::FillBlockPayee -- nBlockHeight=%d, nLevel=-1 failed\n", nBlockHeight);
            return;
        }
    }
    ...
    switch (nLevelMN) {
    case MN_Level1: {
        mnReward = blockReward * 50 / 100;
        break;
    }
    case MN_Level2: {
        mnReward = blockReward * 30 / 100;
        break;
    }
    case MN_Level3: {
        mnReward = blockReward * 15 / 100;
        break;
    }
    case MN_Level4: {
        mnReward = blockReward * 5 / 100;
        break;
    }
    default: {
        mnReward = 0;
    ...
    txNew.vout[0].nValue = (blockReward - mnReward);
    ...
    // ... and masternode
    txoutMasternodeRet = CTxOut(mnReward, payee);
...
{
    if (fLiteMode) return; // disable all PaydayCoin specific functionality
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
        ...
        // PAYDAYCOIN update last vote after sig check
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew, const int64_t& nHeight, const CAmount& fee, CAmount& nTotalRewardWithMasternodes) const
{
    ...
    for (const auto& payee : vecPayees) {
        ...
        for (const auto& txout : txNew.vout) {
            if ((nMasternodePayment * 0.5 == txout.nValue) || (nMasternodePayment * 0.3 == txout.nValue) || (nMasternodePayment * 0.15 == txout.nValue) || (nMasternodePayment * 0.05 == txout.nValue)) {
    ...
    LogPrint(BCLog::MNPAYMENT, "CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f PDAY\n", strPayeesPossible, (float)nMasternodePayment / COIN);
    return false;
}
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman& connman)
{
    ...
    const CScript& payee = GetScriptForDestination(PKHash(mnInfo.pubKeyCollateralAddress));
...
std::string CMasternodePaymentVote::ToString() const
{
    ...
    info << masternodeOutpoint.ToStringShort() << ", " << nBlockHeight << ", " << nStartHeight << ", " << ScriptToAsmStr(payee) << ", " << (int)vchSig.size();
