...
// PAYDAYCOIN This is not a CORS check. Only check if OPTIONS are
...
static bool checkPreflight(HTTPRequest* req)
{
    ...
    // PaydayCoin Keep origin so as not to have to set Access-Control-Allow-Origin
    // to *
    ...
    // unbounded.
    // PaydayCoin We are only satisfying the OPTIONS request in this method in order to
    ...
    if (req->GetRequestMethod() == HTTPRequest::OPTIONS) {
        ...
        // PaydayCoin as we are only handling OPTIONS there is
...
{    
    // PAYDAYCOIN Check if this is a preflight request - if so set the necessary
